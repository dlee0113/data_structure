...
// http://penguin.ewu.edu/class/cscd320/Topic/BSTree/BST/BST.java
...
public class BST {
	 ...
	 * 
	 * data area --- generic Comparable height --- height within the tree of
	 * THIS node size --- number of nodes in this (sub)tree util ---
	 * miscellaneous int value, should we every need it left link --- left
	 * subtree right link --- right subtree
	 * 
	...
	protected BSTnode root = null, current = null; // Spare reference for
													// processing
	...
	double sumDepths(BSTnode node, int deep) {
		if (node == null)
			...
			return deep + sumDepths(node.left, deep + 1)
	...
	/*
	 * Empty out the tree. The "autumn" method recycles the nodes to save on
	...
	BSTnode delete(BSTnode node, Comparable value) {
		...
		if (node == lastNode) {// Check to see if we indeed DID find the value
			if (deleteNode != null && value.equals(deleteNode.data)) {// Final
																		// check:
																		// if
																		// node
																		// is
	...
	void setPos(BSTnode node) {// If the nodes were all printed on one line,
								// their order is based
								// on an in-order traversal. skip shows number
								// of positions to skip
								// to properly position the node. Note that this
								// MUST be a class
								// data member --- the root depends on skip to
								// come back with the
								// size of the entire left subtree, for
								// instance.
		if (node != null) {
			setPos(node.left);
	...
	void pretty(BSTnode node) {// work queue during traversal
	...
	static int newHt(BSTnode node) {
		BSTnode lt = node.left, rt = node.right;
		...
		if (lt == null && rt == null) // Leaf node is height zero
			...
			// Full node --- need java.lang.Math.max
